...
/**
 * The largest single file the server accepts in a multipart request — its
 * `MAX_BUFFERED_FILE_BYTES`, the parser's 6 MiB limit plus one MiB of slack.
 * Multer refuses the WHOLE request with 413 when one file is over it, so a
 * file this size is never sent: it is parked locally, with the reason, before
 * it can poison the ninety-nine files batched beside it.
 */
...
/**
 * The request never got an answer: a refused connection, a reset, a timeout, a
 * DNS miss. That is a fact about the network, not about any file, and it is
 * charged to nothing. An HTTP error IS an answer, and is charged to the batch
 * that drew it.
 */
...
/** The whole multipart request was refused for its size — one file in it does not fit. */
